...
public abstract class VCommand extends Arguments {
	 ...
	 * @param plugin
	 */
	public VCommand(SpawnerPlugin plugin) {
		super();
		this.plugin = plugin;
	}
	 ...
	 * @return
	 */
	public CommandType prePerform(SpawnerPlugin plugin, CommandSender commandSender, String[] args) {
		...
		try {
            ...
            Logger.showException(Config.enableDebug, "Executing the command " + this.getSyntax(), e);
			return CommandType.SYNTAX_ERROR;
		}
	}
	 ...
	 */
	protected abstract CommandType perform(SpawnerPlugin plugin);
	 ...
	 * @return
	 */
	public List<String> toTab(SpawnerPlugin plugin, CommandSender sender, String[] args) {
    ...
    protected List<String> getSpawners(String[] args, int index, SpawnerPlugin plugin, SpawnerType type) {
        if (args == null || args.length <= index) return Collections.emptyList();

        String ownerName = args[index];
