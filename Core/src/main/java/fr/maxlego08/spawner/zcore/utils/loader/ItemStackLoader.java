package fr.maxlego08.spawner.zcore.utils.loader;

import fr.maxlego08.menu.api.itemstack.Potion;
import fr.maxlego08.spawner.exceptions.ItemEnchantException;
import fr.maxlego08.spawner.exceptions.ItemFlagException;
...
public class ItemStackLoader extends ZUtils implements Loader<ItemStack> {
    ...
    public ItemStack load(YamlConfiguration configuration, String path) {
        ...
        if (configuration.contains(path + "url")) {
            ...
            boolean isArrow = configuration.getBoolean(path + "arrow", false);

            item = new Potion(type, level, splash, extended,isArrow).toItemStack(amount);
